...
class TorchCompressedDevice:
    ...
    def compress(self, tensor, comp_config):
        ...
        group_size, num_bits, group_dim, symmetric = (
            comp_config.group_size, comp_config.num_bits,
            comp_config.group_dim, comp_config.symmetric)
        assert group_size % 2 == 0 and not symmetric
        ...
        right_indices = (
            ...
            (slice(1, data.shape[group_dim+1], 2),))
        data = torch.bitwise_or(
            data[left_indices].bitwise_left_shift(num_bits), data[right_indices])
    ...
    def decompress(self, tensor):
        ...
        right_indices = (
            tuple(slice(0, x) for x in data.shape[:group_dim+1]) +
            (slice(1, data.shape[group_dim+1], 2),))
        data[left_indices] = packed.bitwise_right_shift(num_bits)
        data[right_indices] = packed.bitwise_and(2 ** num_bits - 1)
...
def compress(tensor, config):
    ...
    group_size, num_bits, group_dim, symmetric = (
        config.group_size, config.num_bits,
        config.group_dim, config.symmetric)
    ...
    # Reshape
    data_shape = (
        original_shape[:group_dim] + (num_groups * (group_size // 2),) + original_shape[group_dim+1:])
    scale_shape = (
        original_shape[:group_dim] + (num_groups, 2) + original_shape[group_dim+1:])
    data = data.view(data_shape)
    scale = torch.cat([scale, mn], dim=group_dim+1).view(scale_shape)
...
def decompress(packed_data, config):
    ...
    # Unpack
    data, scale, original_shape = packed_data
    left_bits = data >> num_bits
    right_bits = data & (2 ** num_bits - 1)

    # Dequantize
    scale, mn = scale
    data = (left_bits.float() * scale) + mn
